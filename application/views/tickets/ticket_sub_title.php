...
if (count($custom_fields_list)) {
    ...
    foreach ($custom_fields_list as $data) {
        if ($data->value) {
            $fields.= "<div class='p15 bg-white b-b '><i class='fa fa-check-square ml15'></i> <span class='text-off'> $data->custom_field_title:</span> " . $this->load->view("custom_fields/output_" . $data->custom_field_type, array("value" => $data->value), true) . "</div>";
        }
    }
    if ($fields) {
        echo $fields;
