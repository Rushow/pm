...
class Team_point extends MY_Controller {
    ...
    function index() {
        ...
        $this->load->model('TeamPoint_model');
        $view_data['datas'] = $this->TeamPoint_model->allData()->result();
        
         
        $access_info = $this->get_access_info("invoice");
        $view_data["show_invoice_info"] = (get_setting("module_invoice") && $access_info->access_type == "all") ? true : false;
        $this->template->rander("team_points/index", $view_data);

    }


    // get records using month search
    ...
    function getRecords() {
        ...
        $view_data['data'] = $this->TeamPoint_model->getRecords($optionMonth, $optionYear)->result();
        $this->access_only_allowed_members();
        ...
        $this->template->rander("team_points/points", $view_data);
    ...
    function points() {
        ...
        $this->load->model('TeamPoint_model');
        $view_data['marks'] = $this->TeamPoint_model->fetchMarks(date('m'),date('Y'))->result();
        
         
        //$this->load->view('team_points/test', $view_data);
    ...
    function modal_form() {
        ...
        //get custom fields
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("clients", $client_id)->result();

        $this->load->view('team_points/modal_form', $view_data);
    }
    ...
    /* load Point add/edit modal */

    function point_modal_form() {


         $this->load->model('TeamPoint_model');
         $view_data['user_datas'] = $this->TeamPoint_model->users_form_data()->result();
         $view_data['category_datas'] = $this->TeamPoint_model->categories_form_data()->result();
        ...
        $this->load->view('team_points/point_modal_form', $view_data);
   ...
   function get_currency_dropdown_select2_data() {
    ...
    function save() {
           ...
           if(isset($is_submit) && $is_submit == 1){
              ...
              $this->TeamPoint_model->create($options);
              redirect('/team_point', 'location');
        }
    }
    ...
    function savePoint() {
           ...
           if(isset($is_submit) && $is_submit == 1){
              ...
              $this->TeamPoint_model->createPoint($options);
              redirect('/team_point', 'location');
        }
    }
     ...
     function edit_modal_form($id) {
        ...
        $this->load->model('TeamPoint_model');
        $view_data['fetch_data'] = $this->TeamPoint_model->fetch_single_data($user_id)->result();
        $view_data['category_data'] = $this->TeamPoint_model->categories_form_data()->result();
        ...
        $this->load->view('team_points/edit_modal_form', $view_data);
    }
    ...
    function updatePoint($id) {
           ...
           if(isset($is_submit) && $is_submit == 1) {
              ...
              $this->TeamPoint_model->update($options, $id);
              redirect('/team_point', 'location');
    ...
    function deletePoint($id) {
           ...
           $this->load->model('TeamPoint_model');
           $this->TeamPoint_model->deletePoint($id);
              redirect('/team_point', 'location');
    ...
    function list_data() {
        ...
        $this->load->model('TeamPoint_model');
        $view_data = $this->TeamPoint_model->fetchMarks(date('m'),date('Y'))->result();
    ...
    private function _row_data($id) {
        ...
        $options = array("id" => $id);
        $data = $this->TeamPoint_model->get_details($options)->row();
    ...
    private function _make_row($data) {
        ...
        return array($data->id,
            anchor(get_uri("team_point/view/" . $data->id), $data->first_name),
            $data->last_name, $data->mark, 
            modal_anchor(get_uri("team_point/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_client'), "data-post-id" => $data->id))
    ...
    function add_new_contact_modal_form() {
        ...
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("contacts", $view_data['model_info']->id)->result();
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("contacts", $contact_id)->result();
    ...
    function save_contact() {
        ...
        if ($save_id) {
            ...
            save_custom_fields("contacts", $save_id);
