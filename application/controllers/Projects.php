...
class Projects extends MY_Controller {
    ...
    function modal_form() {
        ...
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("projects", $view_data['model_info']->id)->result();
    ...
    function save() {
        ...
        if ($save_id) {
            ...
            save_custom_fields("projects", $save_id);
    ...
    function overview($project_id) {
        ...
        $view_data['custom_fields_list'] = $this->Custom_field_values_model->get_details(array("related_to_type" => "projects", "related_to_id" => $project_id))->result();
    ...
    function timer_note_modal_form() {
        ...
        $id = $this->input->post("id");
        if ($id) {
            $view_data["model_info"] = $this->Timesheets_model->get_one($id);
    ...
    function timesheet_list_data($project_id = 0) {
        $this->access_only_team_members();
        $options = array("project_id" => $project_id, "status" => "none_open", "user_id" => $this->input->post("user_id"), "start_date" => $this->input->post("start_date"), "end_date" => $this->input->post("end_date"), "task_id" => $this->input->post("task_id"));
    ...
    function tasks_list_data($project_id = 0) {
        ...
        $milestone_id = $this->input->post('milestone_id');
        $options = array("project_id" => $project_id, "assigned_to" => $this->input->post('assigned_to'), "deadline" => $this->input->post('deadline'), "status" => $status, "milestone_id" => $milestone_id);
    ...
    function my_tasks_list_data() {
        ...
        $options = array("specific_user_id" => $specific_user_id, "status" => $status, "project_id" => $project_id, "deadline" => $this->input->post('deadline'));
